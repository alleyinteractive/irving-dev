...
class Partials {
	...
	public function load( $args ) {
		...
		$cache = ! empty( $args['cache'] ) && ! is_customize_preview();
		...
		/**
		 * Execute an action before rendering the partial.
		 *
		 * @param array $args Options for loading the partial.
		 */
		do_action( 'ai_partials_before_load_template', $args );
		...
		if ( isset( $args['loop'] ) ) {
			...
			$this->load_single( $args );
		}

		$this->restore_post();

		/**
		 * Execute an action after rendering the partial.
		 *
		 * @param array $args Options for loading the partial.
		 */
		do_action( 'ai_partials_after_load_template', $args );
