...
class PartialsTest extends WP_UnitTestCase {
	...
	public function test_nested_loops() {
		...
		$this->assertSame( $posts[0], get_the_ID() );
		$this->assertEmpty( \Irving_Example\Partials::instance()->original_posts );
	}
	...
	public function test_stylesheet_support() {
		if ( class_exists( '\Alleypack\Stylesheets' ) && function_exists( 'ai_use_stylesheet' ) ) {
			// Set up the Stylesheets module.
			\Alleypack\Stylesheets::instance()->setup( ALLEYPACK_PATH . '/tests/stylesheets/mocks/mock-classnames.json' );

			// Set the initial stylesheet value.
			ai_use_stylesheet( 'mock-one' );

			// Test ai_get_template_part, in which args are an array passed with other parameters.
			$get_template_part_contents = $this->ob( function() {
				ai_get_template_part(
					'tests/partials/template-parts/stylesheets',
					[ 'stylesheet' => 'mock-two' ]
				);
			} );

			// Verify the partial uses the passed stylesheet.
			$this->assertContains( 'Template loaded: mock-two', $get_template_part_contents );
			// Verify the stylesheet is reset to the original value.
			$this->assertSame( 'mock-one', \Alleypack\Stylesheets::instance()->current_stylesheet );

			// Test ai_partial, in which parameters are a flat array.
			$partial_contents = $this->ob( function() {
				ai_partial(
					[
						'slug'       => 'tests/partials/template-parts/stylesheets',
						'stylesheet' => 'mock-three',
					]
				);
			} );

			// Verify the partial uses the passed stylesheet.
			$this->assertContains( 'Template loaded: mock-three', $partial_contents );
			// Verify the stylesheet is reset to the original value.
			$this->assertSame( 'mock-one', \Alleypack\Stylesheets::instance()->current_stylesheet );
		}
	}
