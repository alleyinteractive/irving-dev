 ...
 */
class PostSelector extends React.PureComponent {
  /**
  ...
  render() {
    ...
    return (
      <form>
        <TextControl
          label={__('Search Text', 'alleypack')}
        ...
        {false === loading && '' !== searchText && 0 === foundPosts.length && (
          <div>{__('No matching posts found.', 'alleypack')}</div>
        )}
        {false === loading && 0 < foundPosts.length && (
          <SelectControl
            label={__('Selected Post', 'alleypack')}
            onChange={this.handlePostSelect}
            options={[
              {
                label: __('Select post', 'alleypack'),
                value: '',
              },
              ...foundPosts.map((post) => ({
