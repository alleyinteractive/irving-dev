...
class Schedule_Unpublish {
	 ...
	 * @var string
	 */
	const META_KEY = 'alleypack_schedule_unpublish';
	...
	/**
	 * Post types for which this module should be activated.
	 *
	 * @var array
	 */
	...
	public function setup() {
		...
		// Allow filtering of post types using this module.
		add_action(
			'init',
			function() {
				$this->post_types = apply_filters( 'alleypack_schedule_unpublish_post_types', $this->post_types ); // phpcs:ignore WordPressVIPMinimum.Variables.VariableAnalysis.UndefinedVariable
			}
		);
	...
	public function add_classic_editor_meta_field( $type ) {
		...
		// Bail if the post type does not use this module.
	...
	public function action_admin_enqueue_scripts() {
		...
		// Bail if the post type does not use this module.
		if ( ! in_array( $current_screen->post_type ?? '', $this->post_types, true ) ) {
			return;
		}

		wp_enqueue_script(
			'schedule-unpublish',
			plugins_url( '1.0/build/scheduleUnpublish.js', __DIR__ ),
