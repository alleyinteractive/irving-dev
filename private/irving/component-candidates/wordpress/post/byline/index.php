			...
			$component
				->append_child(
					( new Component( 'irving/link' ) )
						->set_config( 'href', '/author/yeah/' )
						->set_child( get_the_author_meta( 'display_name', $post->post_author ) )
				)
				->append_child(
					( new Component( 'irving/text' ) )->set_config( 'content', 'James Burke' )
				)
				->append_child(
					( new Component( 'irving/text' ) )->set_config( 'content', 'Amy Evans' )
				)
				->append_child(
					( new Component( 'irving/text' ) )->set_config( 'content', 'James Burke' )
				)
				->append_child(
					( new Component( 'irving/text' ) )->set_config( 'content', 'Owie Stowie' )
				);
