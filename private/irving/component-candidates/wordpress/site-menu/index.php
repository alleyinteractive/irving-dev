...
\WP_Irving\get_registry()->register_component_from_config(
	...
	[
		'callback' => function( Component $component ) {
			...
			$component->set_children( wp_irving_convert_menu_to_components( $menu_items ) );
 ...
 * @return array
 */
function wp_irving_convert_menu_to_components( array $menu_items, $parent_id = 0 ) {
	...
	foreach ( $menu_items as $menu_item ) {
		...
		if ( $menu_item->get_config( 'parent_id' ) === $parent_id ) {
			$menu_item->set_children( wp_irving_convert_menu_to_components( $menu_items, $menu_item->get_config( 'id' ) ) );
			$menu[] = $menu_item;
		}
	}
